  ...
  const loadUsuario = async () => {
    try {
      ...
      if (data.success) {
        ...
        setUsuario(user);
        setOriginalTipo(user.tipo_usuario);
        ...
        // Si es empleado, cargar los detalles del empleado
        if (user.empleado_detalles) {
          setEmpleadoData({
            cargo: user.empleado_detalles.cargo || '',
            salario: user.empleado_detalles.salario?.toString() || '',
            fecha_contratacion: user.empleado_detalles.fecha_contratacion || '',
            turno: user.empleado_detalles.turno || 'mañana',
            activo: user.empleado_detalles.activo || false,
          });
        }
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setErrors({ general: data.error || 'Error al actualizar el usuario' });
      }
    } catch {
          ...
          <div className="bg-white/5 backdrop-blur-sm rounded-xl p-6 border border-white/10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {originalTipo !== formData.tipo_usuario && (
                  <p className="text-yellow-400 text-sm mt-1">
                    ⚠️ Cambiar el tipo de usuario puede afectar los permisos y funcionalidades
                  </p>
                )}
              </div>
